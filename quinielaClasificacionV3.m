...
%%
%%Quiniela mediante clasificación midiendo la distancia media a K centroides
%%
function yEstim = quinielaClasificacionV3(x)
...
for i=1:size(x,2)
    ...
    dato = [v1 v2 s1 s2 f1 f2]';
      ...
      mediaDistanciasClase1 = sum(distancias(1:K))/K;
      mediaDistanciasClase2 = sum(distancias(K+1:2*K))/K;
      mediaDistanciasClase3 = sum(distancias((2*K)+1:3*K))/K;
